...
contract AmmModule is ModuleBase, ReentrancyGuard {
    ...
    function removeLiquidity(
        ...
        _executeLiquidityTokenApprovals(actionInfo);
    ...
    function _getActionInfo(
        ...
        actionInfo.ammAdapter = IAmmAdapter(
            getAndValidateAdapter(_integrationName)
        );

        actionInfo.liquidityToken = _ammPool;

        actionInfo.preActionLiquidityTokenBalance = IERC20(_ammPool).balanceOf(
            address(_setToken)
        );

        actionInfo.preActionComponentBalances = _getTokenBalances(
            address(_setToken),
            _components
        );

        actionInfo.liquidityQuantity = actionInfo
            .totalSupply
            .getDefaultTotalNotional(_poolTokenInPositionUnit);

        actionInfo.totalNotionalComponents = _getTotalNotionalComponents(
            _setToken,
            _componentUnits
        );
    ...
    function _validateAddLiquidity(ActionInfo memory _actionInfo)
        internal
        view
    {
        ...
        for (uint256 i = 0; i < _actionInfo.components.length; i++) {
            ...
            require(
                _actionInfo.setToken.hasSufficientDefaultUnits(
                    component,
                    _actionInfo.componentUnits[i]
                ),
    ...
    function _validateRemoveLiquidity(ActionInfo memory _actionInfo)
        internal
        view
    {
        _validateCommon(_actionInfo);
        ...
        for (uint256 i = 0; i < _actionInfo.components.length; i++) {
            require(
                _actionInfo.componentUnits[i] > 0,
                "Component quantity must be nonzero"
            );
        }
    ...
    function _validateCommon(ActionInfo memory _actionInfo) internal view {
        require(
            _actionInfo.componentUnits.length == _actionInfo.components.length,
            "Components and units must be equal length"
        );

        require(
            _actionInfo.liquidityQuantity > 0,
            "Token quantity must be nonzero"
        );
    ...
    function _executeApprovals(ActionInfo memory _actionInfo) internal {
        address spender =
            _actionInfo.ammAdapter.getSpenderAddress(
                _actionInfo.liquidityToken
            );
    ...
    function _executeLiquidityTokenApprovals(ActionInfo memory _actionInfo)
        internal
    {
        address spender =
            _actionInfo.ammAdapter.getSpenderAddress(
    ...
    function _executeAddLiquidity(ActionInfo memory _actionInfo) internal {
        ...
        uint256 lpBalance =
            IERC20(_actionInfo.liquidityToken).balanceOf(address(this));
        IERC20(_actionInfo.liquidityToken).transfer(
            address(_actionInfo.setToken),
            lpBalance
        );
    }

    function _executeAddLiquiditySingleAsset(ActionInfo memory _actionInfo)
        internal
    {
        (address targetAmm, uint256 callValue, bytes memory methodData) =
    ...
    function _executeRemoveLiquidity(ActionInfo memory _actionInfo) internal {
        ...
        for (uint256 i = 0; i < _actionInfo.components.length; i++) {
            uint256 tokenBalance =
                IERC20(_actionInfo.components[i]).balanceOf(address(this));
            IERC20(_actionInfo.components[i]).transfer(
                address(_actionInfo.setToken),
                tokenBalance
            );
        }
    ...
    function _executeRemoveLiquiditySingleAsset(ActionInfo memory _actionInfo)
        internal
    {
        (address targetAmm, uint256 callValue, bytes memory methodData) =
    ...
    function _validateMinimumLiquidityReceived(ActionInfo memory _actionInfo)
        internal
        view
    {
        uint256 liquidityTokenBalance =
            IERC20(_actionInfo.liquidityToken).balanceOf(
                address(_actionInfo.setToken)
            );

        require(
            liquidityTokenBalance >=
                _actionInfo.liquidityQuantity.add(
    ...
    function _validateMinimumUnderlyingReceived(ActionInfo memory _actionInfo)
        internal
        view
    {
        for (uint256 i = 0; i < _actionInfo.components.length; i++) {
            uint256 underlyingBalance =
                IERC20(_actionInfo.components[i]).balanceOf(
                    address(_actionInfo.setToken)
                );

            require(
                underlyingBalance >=
                    _actionInfo.totalNotionalComponents[i].add(
    ...
    function _updateComponentPositions(ActionInfo memory _actionInfo)
        internal
        returns (int256[] memory)
    {
        int256[] memory componentsReceived =
            new int256[](_actionInfo.components.length);
        ...
        for (uint256 i = 0; i < _actionInfo.components.length; i++) {
            ...
            componentsReceived[i] = currentComponentBalance.toInt256().sub(
                _actionInfo.preActionComponentBalances[i].toInt256()
            );
        }
    ...
    function _updateLiquidityTokenPositions(ActionInfo memory _actionInfo)
        internal
        returns (int256)
    {
        (uint256 currentLiquidityTokenBalance, , ) =
            _actionInfo.setToken.calculateAndEditDefaultPosition(
                _actionInfo.liquidityToken,
                _actionInfo.totalSupply,
                _actionInfo.preActionLiquidityTokenBalance
        ...
        return
            currentLiquidityTokenBalance.toInt256().sub(
                _actionInfo.preActionLiquidityTokenBalance.toInt256()
            );
    }
    ...
    function _getTotalNotionalComponents(
        ...
        uint256[] memory totalNotionalQuantities =
            new uint256[](_tokenAmounts.length);
        for (uint256 i = 0; i < _tokenAmounts.length; i++) {
            totalNotionalQuantities[i] = Position.getDefaultTotalNotional(
                totalSupply,
                _tokenAmounts[i]
            );
        }
